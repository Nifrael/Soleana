...
const server = {
  contact: defineAction({
    input: z.object({
      name: z.string().min(3, "Le nom doit contenir au moins 3 caractères.").regex(nameRegex, "Le nom contient des caractères non autorisés."),
      email: z.string().email("L'adresse e-mail n'est pas valide."),
      message: z.string().min(20, "Le message doit contenir au moins 20 caractères."),
      phoneNumber: z.string().regex(phoneRegex, "Le format du numéro de téléphone est invalide.").optional()
    }),
    handler: async (input) => {
